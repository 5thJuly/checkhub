...
export function createFoodSelector<T extends PricedMeal>(population: T[], target = TARGET_LUNCH_PRICE) {
    ...
    return {
        probabilities, expectedPrice: mean(raw), meanFor(items: T[]) { const { w, sum } = weighted(items); return items.reduce((s, f, i) => s + f.price * w[i], 0) / sum }, choose(items: T[], random = Math.random): T {
...
export function chooseWeightedFood<T extends { rarity: number }>(items: T[], random = Math.random): T {
    ...
    for (const item of items) {
        const r = Math.min(4, Math.max(0, item.rarity ?? 0));
    ...
    for (const item of items) {
        const r = Math.min(4, Math.max(0, item.rarity ?? 0));
        const tierWeight = CSGO_TIER_WEIGHTS[r] ?? 0.0026;
        const count = counts.get(r) || 1;
        const w = tierWeight / count;
        weights.push(w);
        totalWeight += w;
    }
